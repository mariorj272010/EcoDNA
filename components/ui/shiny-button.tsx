...
interface ShinyButtonProps { children: ReactNode; onClick?: () => void; className?: string; }
...
export function ShinyButton({ children, onClick, className = "" }: ShinyButtonProps) {
  return <>
    <style jsx>{`
      ...
      .shiny-cta {
        --shiny-cta-bg: var(--card-paper, #f7f4ea); --shiny-cta-bg-subtle: var(--hairline, #d3c9ab);
        --shiny-cta-fg: var(--ink, #211d16); --shiny-cta-highlight: var(--flag-orange, #e4590f);
        --shiny-cta-highlight-subtle: #f4885b; --animation: gradient-angle linear infinite; --duration: 3s;
        --shadow-size: 2px; --transition: 800ms cubic-bezier(.25,1,.5,1);
        isolation: isolate; position: relative; overflow: hidden; cursor: pointer; outline-offset: 4px;
        padding: 16px 28px; font-family: var(--font-mono, monospace); font-size: var(--text-xs, .6875rem);
        line-height: 1.2; font-weight: 700; text-transform: uppercase; letter-spacing: .04em;
        border: 1px solid transparent; border-radius: var(--radius-sm, 3px); color: var(--shiny-cta-fg);
        ...
        box-shadow: inset 0 0 0 1px var(--shiny-cta-bg-subtle); transition: var(--transition);
      ...
      .shiny-cta::before { --size:calc(100% - var(--shadow-size) * 3); --position:2px; --space:calc(var(--position) * 2); width:var(--size); height:var(--size); background:radial-gradient(circle at var(--position) var(--position),var(--ink,#211d16) calc(var(--position) / 4),transparent 0) padding-box; background-size:var(--space) var(--space); background-repeat:space; mask-image:conic-gradient(from calc(var(--gradient-angle) + 45deg),black,transparent 10% 90%,black); border-radius:inherit; opacity:.4; }
      .shiny-cta::after { --animation:shimmer linear infinite; width:100%; aspect-ratio:1; background:linear-gradient(-50deg,transparent,var(--shiny-cta-highlight),transparent); mask-image:radial-gradient(circle at bottom,transparent 40%,black); opacity:.6; }
      ...
      .shiny-cta,.shiny-cta::before,.shiny-cta::after { animation:var(--animation) var(--duration),var(--animation) calc(var(--duration) / .4) reverse paused; animation-composition:add; }
    ...
    `}</style>
    <button className={`shiny-cta ${className}`} onClick={onClick}><span>{children}</span></button>
  </>;
}
